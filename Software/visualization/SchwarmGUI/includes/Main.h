...
namespace Main
{
    ...
    bool any_key_pressed(GLFWwindow* window)
    {
        return (glfwGetKey(window, CHANGE_VIEW)     == GLFW_PRESS ||
                glfwGetKey(window, PAUSE_KEY)       == GLFW_PRESS);
    }

    void process_key_events(GLFWwindow* window)
    {
        static bool key_pressed = false;
        static Location old_location = cam_loc;

        if(!key_pressed && glfwGetKey(window, CHANGE_VIEW) == GLFW_PRESS)
        {
        ...
        if(!key_pressed && glfwGetKey(window, PAUSE_KEY) == GLFW_PRESS && view_state != ViewState::CAMERA_VIEW)
        {
        ...
        if(key_pressed && !any_key_pressed(window))
            key_pressed = false;
    }
